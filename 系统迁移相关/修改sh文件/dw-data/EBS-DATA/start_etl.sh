#! /bin/bash
# path location 
path="/oracle/dw-data/EBS-DATA"
cd /oracle/dw-data/EBS-DATA
echo "strat to load data for csv document"
echo "date is " $1
DOCUMENTNAME="$1-EBS-DATA.READY"
echo "$DOCUMENTNAME will be used for etl" >>"$path"/error.log
T=`date +%D:%r`:
echo "NOW: $T"

# CONTROL FILE PATH
ctl_path="/oracle/dw-data/EBS-DATA"
echo " ">>"$path"/error.log
echo " ">>"$path"/error.log
echo "***********************">>"$path"/error.log
...
echo "***********************">>"$path"/error.log
echo "$path/$DOCUMENTNAME" >>"$path"/error.log
echo `pwd` >>"$path"/error.log
...
EOF
  ...
  echo " ">>"$path"/error.log
  #ONLOAD TABLE EBS_PER_ALL_PEOPLE_F
  echo "$T begin to load table EBS_PER_ALL_PEOPLE_F"
  echo "[INFO] $T begin to load table EBS_PER_ALL_PEOPLE_F">>"$path"/error.log
  # UPDATE THE PROPERTY CSV DOCUMENT TO ADAPT HAVING DOCUMENT
  sed -i '3d' "$ctl_path"/control/EBS_PER_ALL_PEOPLE_F.ctl 
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1_per_all_people_f_$period.csv\""" "$ctl_path"/control/EBS_PER_ALL_PEOPLE_F.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
  sqlldr src_layer/oracle@ICBCAMCDW control="$ctl_path"/control/EBS_PER_ALL_PEOPLE_F.ctl bad="$path/$1"/bad/EBS_PER_ALL_PEOPLE_F.bad log="$path/$1"/log/EBS_PER_ALL_PEOPLE_F.log>>"$path"/error.log
    # IS SUCCESS OR NOT 
  if [ -f "$path/$1"/bad/EBS_PER_ALL_PEOPLE_F.bad ];then  
  is_success="no"
  else
  is_success="yes"
  fi
...
values 
('src_layer',
'$path/$1/csv/$1_per_all_people_f.csv',
'src_layer.EBS_PER_ALL_PEOPLE_F',
(select data_date from etl_control.ctl_general_parameter where rownum=1),
sysdate,
'$is_success',
'$path/$1/log/EBS_PER_ALL_PEOPLE_F.log',
'$path/$1/bad/EBS_PER_ALL_PEOPLE_F.bad');
commit;
exit;
EOF
  echo "*************end***************">>"$path"/error.log
