...
echo "NOW: $T"
# path location 
path="/oracle/dw-data/AICS"
# CONTROL FILE PATH
ctl_path="/oracle/dw-data/AICS"
echo " ">>"$path"/error.log
echo " ">>"$path"/error.log
echo "***********************">>"$path"/error.log
...
EOF
  ...
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-TCUSTOMERINFO.csv\""" "$ctl_path"/control/AICS_CUSTOMERINFO.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
...
values 
('src_layer',
'$path/$1/csv/$1-TCUSTOMERINFO.csv',
...
EOF
  ...
  # ONLOAD TABLE AICS_EQUITY_ASSETS_DETAIL
  echo "$T begin to load table AICS_EQUITY_ASSETS_DETAIL"
  echo "[INFO] $T begin to load table AICS_EQUITY_ASSETS_DETAIL">>"$path"/error.log
  # UPDATE THE PROPERTY CSV DOCUMENT TO ADAPT HAVING DOCUMENT
  sed -i '3d' "$ctl_path"/control/AICS_EQUITY_ASSETS_DETAIL.ctl 
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-EquityAssetDetail.csv\""" "$ctl_path"/control/AICS_EQUITY_ASSETS_DETAIL.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
  sqlldr src_layer/oracle@ICBCAMCDW control="$ctl_path"/control/AICS_EQUITY_ASSETS_DETAIL.ctl bad="$path/$1"/bad/AICS_EQUITY_ASSETS_DETAIL.bad log="$path/$1"/log/AICS_EQUITY_ASSETS_DETAIL.log>>"$path"/error.log
    # IS SUCCESS OR NOT 
  if [ -f "$path/$1"/bad/AICS_EQUITY_ASSETS_DETAIL.bad ];then  
  is_success="no"
  else
  is_success="yes"
  fi
...
values 
('src_layer',
'$path/$1/csv/$1-EquityAssetDetail.csv',
'src_layer.AICS_EQUITY_ASSETS_DETAIL',
(select data_date from etl_control.ctl_general_parameter where rownum=1),
sysdate,
'$is_success',
'$path/$1/log/AICS_EQUITY_ASSETS_DETAIL.log',
'$path/$1/bad/AICS_EQUITY_ASSETS_DETAIL.bad');
commit;
exit;
EOF
  echo "*************end***************">>"$path"/error.log
...
EOF
  ...
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-TFUNDINFO.csv\""" "$ctl_path"/control/AICS_FUNDINFO.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
...
EOF
  ...
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-TTRUSTCONTRACTDETAILS.csv\""" "$ctl_path"/control/AICS_TRUSTCONTRACTDETAILS.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
...
values 
('src_layer',
'$path/$1/csv/$1-TTRUSTCONTRACTDETAILS.csv',
...
EOF
  ...
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-TSTATICSHARES.csv\""" "$ctl_path"/control/AICS_STATICSHARES.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
...
values 
('src_layer',
'$path/$1/csv/$1-TSTATICSHARES.csv',
...
EOF
  ...
  # INSERT TRUE CONFIG FOR CONTROL DOCUMENT
  sed -i "/LOAD DATA/aINFILE "\"$path/$1/csv/$1-TCONFIRM.csv\""" "$ctl_path"/control/AICS_CONFIRM.ctl
  echo "************begin**************">>"$path"/error.log  
  # SQLLDR FOR LOADDATA
